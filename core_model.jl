...
function ode_system!(du, u, p, t)
    ...
    @inline rho_fun = i -> 2*mapreduce(j -> p.f[j,i] * B[j] * p.r_x[j], +, i:index_end) * carrying_coefficient
    ...
    @inline death_rate = x -> p.r_x[x]*B[x]*carrying_coefficient
    ...
    unbound_targets = mapreduce(x -> free_target_number(x) * B[x],+, eachindex(B))
    bound_targets = mapreduce(x -> bound_target_number(x) * B[x], +, eachindex(B))
    free_targets_released = mapreduce(x -> p.d_x[x]*free_target_number(x) * B[x], +, eachindex(B))
    bound_targets_released = mapreduce(x -> p.d_x[x]*bound_target_number(x) * B[x], +, eachindex(B))
    dA = -binding_coefficient * (A*T + A*unbound_targets) +  
    p.k_r * (AT +  bound_targets)
    dT = -binding_coefficient * A*T + p.k_r * AT + free_targets_released
    dAT = binding_coefficient * A*T - p.k_r * AT + bound_targets_released
end
